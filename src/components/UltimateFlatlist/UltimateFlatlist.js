...
import UltimateFlatlistRow from './UltimateFlatlistRow';

/**
 * ListView that renders UltimateFlatlistRows.
 */
class UltimateFlatlist extends PureComponent {
  ...
  manuallyOpenAllRows(toValue) {
    Object.keys(this._rows).forEach(rowKey => {
      const row = this._rows[rowKey];
      if (row && row.manuallyUltimateFlatlistRow) {
        row.manuallyUltimateFlatlistRow(toValue);
      }
    });
  }
    ...
    if (!HiddenComponent) {
      ...
      return (
        <UltimateFlatlistRow
          ...
          style={this.props.UltimateFlatlistRowStyle}
...
UltimateFlatlist.propTypes = {
   ...
   * This is required unless renderItem is passing a UltimateFlatlistRow.
   */
   ...
   * One case is a UltimateFlatlist with rows of different heights and an options to delete rows.
   */
  ...
  /**
   * Called when scrolling on the UltimateFlatlist has been enabled/disabled
   */
  ...
  /**
   * Styles for the parent wrapper View of the UltimateFlatlistRow
   */
  UltimateFlatlistRowStyle: PropTypes.object,
  /**
   * Called when the FlatList ref is set and passes a ref to the FlatList
   * e.g. listViewRef={ ref => this._UltimateFlatlistRef = ref }
   */
  ...
  /**
   * [DEPRECATED] Should the first UltimateFlatlistRow do a slide out preview to show that the list is swipeable
   */
  ...
  /**
   * Callback invoked any time the swipe value of a UltimateFlatlistRow is changed
   */
